...
import { DeliveryOrderCreateByTraceAsync, DeliveryOrderGetAsync } from "@/services/wms/DeliveryOrder";
...
import BindTraceIdDialog from "./components/bindTraceIdDialog";
import { pubGoBack } from "@/components/public";
import { useKeepAliveParams } from '@/hooks';

const CreateByTraceIdPage = (props: any) => {
    ...
    const handleSubmit = () => {
        return form.submit((values) => {
            if (id) {
                ...
                return DeliveryOrderCreateByTraceAsync(values);
            }
        }).then(() => { pubGoBack() }, () => { });
    }
    ...
    const form = useMemo(() => createForm({
        effects: () => {
            ...
            onFieldValueChange("{value:wareHouseId,label:wareHouseName}", (field, form) => {
                form.setFieldState('items', async (state) => {
                    state.componentProps = {
                        title: () => <BindTraceIdDialog onSelected={onLPNSelected} warehouseId={field.value.value} />
                    }
                })
            })
        },
    ...
    const SchemaField = useMemo(() => createSchemaField({
        components: {
            ...
            WareHouseSelect, MaterailItemSelect, CustomerSelect, ShipmentOrderTypeSelect, ContactInfoSelect
        },
    }), []);

    const onLPNSelected = (rows: any[]) => {
        const result = groupBy(rows, (x) => x.materialId!)
        const items = Object.keys(result).map((key) => {
            const item = result[key];
            return {
                ...item[0],
                materialOutCode: item[0].material.outCode,
                materialDescript: item[0].material.descript,
                quantity: sumBy(item, "qty")
            }
        })
    ...
    return (<PageContainer fixHeader={true} header={{ title: "送货单", subTitle: "创建送货单" }}>
        <Form form={form} layout={"horizontal"} labelWidth={110} feedbackLayout={"terse"} previewTextPlaceholder={"无"}>
                    ...
                    <SchemaField.Void x-component="FormGrid" x-component-props={{ maxColumns: [1, 4], strictAutoFit: true }}>
                        ...
                        <SchemaField.Array name="items" title="送货明细" x-component="ArrayTable" x-validator={[{ message: "送货明细必须填写" }]} x-decorator="FormItem"
                            x-decorator-props={{ gridSpan: 4 }} x-component-props={{
                                title: () => <BindTraceIdDialog onSelected={onLPNSelected} />
                            }}
                        >
                            <SchemaField.Object>
                                <SchemaField.Void x-component="ArrayTable.Column" x-component-props={{ width: 150, title: '出库指令单', fixed: "left" }}>
                                    <SchemaField.String name={"shipmentOrderNo"} required x-decorator="FormItem" x-component="Input" x-read-pretty={true} />
                                    <SchemaField.String name={"shipmentOrderItemId"} required x-decorator="FormItem" x-component="Input" x-hidden={true} />
                                </SchemaField.Void>
                                ...
                                <SchemaField.Void x-component="ArrayTable.Column" x-component-props={{ width: 150, title: '物料编码', fixed: "left" }}>
                                    <SchemaField.String name={"materialOutCode"} required x-decorator="FormItem" x-component="Input" x-read-pretty={true} />
                                ...
                                <SchemaField.Void x-component="ArrayTable.Column" x-component-props={{ title: '物料描述', fixed: "left", ellipsis: true }}>
                                    <SchemaField.String name={"materialDescript"} x-component="Input" x-decorator="FormItem" x-component-props={{ placeholder: "请输入物料描述" }} x-read-pretty={true} />
                                </SchemaField.Void>
            ...
            {
                	<ToolBar style={{zIndex:window.location.pathname.indexOf('create')!=-1?0:-1}}>
...
export default CreateByTraceIdPage
