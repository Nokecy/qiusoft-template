...
import { ProjectRiskGetAsync, ProjectRiskApproveResolutionAsync, ProjectRiskRejectResolutionAsync } from '@/services/pdm/ProjectRisk';
...
const ExecuteFormPage = (props: any) => {
  const { params } = useKeepAliveParams('/appPdm/ProjectManagement/RiskList/execute', ['definitionId', 'correlationId', 'activityId', 'workflowInstanceId', 'workItemId']);
        ...
        effects: () => {
          onFormInit(async (form) => {
            if (correlationId) {
              setLoading(true);
              try {
                let values: any = await ProjectRiskGetAsync({ id: correlationId });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (executeType === 5) {
        // 审批通过
        await ProjectRiskApproveResolutionAsync({
          id: correlationId,
          remark: remark || '审批通过',
        });
        ...
        await ProjectRiskRejectResolutionAsync({
          id: correlationId,
          rejectReason: remark,
          remark: remark,
        });
  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', padding: '16px' }}>
      ...
      {/* 基本信息卡片 */}
      <Card title="风险信息" style={{ marginBottom: 16 }}>
