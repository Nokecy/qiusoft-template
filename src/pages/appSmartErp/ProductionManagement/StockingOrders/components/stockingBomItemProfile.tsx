import {
	StockingOrderGetStockingBomItemListAsync,
	StockingOrderRemoveBomItemAsync,
} from '@/services/smarterp/StockingOrder';
...
import CreateFormDialog from './createStockingBomItemDialog';
import UpdateFormDialog from './updateStockingBomItemDialog';

const StockingBomItemProfile = (props: any) => {
	const { stockingOrderId, stockingOrderItemId } = props;
	...
	const { data, run, loading } = useRequest(
		id => {
			return StockingOrderGetStockingBomItemListAsync({ stockingOrderItemId: id });
		},
		{
			manual: true,
		}
	...
	const loadData = () => {
		run(stockingOrderItemId);
	};

	const handleDelete = id => {
		StockingOrderRemoveBomItemAsync({ bomItemId: id }).then(() => {
			handleRefresh();
		});
	};
	...
	return (
		<>
			<Button onClick={showModal} icon={<LinkOutlined />} title={'配置清单'} type='link' size='small' />
			<Drawer
				width={1200}
				placement='right'
				closable={true}
			...
			>
				<AgGridPlus
					gridRef={gridRef}
					headerTitle='物料清单'
					gridKey="stocking-bom-item-tree"
					...
					toolBarRender={() => [
						<CreateFormDialog
							...
							stockingOrderItemId={stockingOrderItemId}
							buttonProps={{
								icon: <PlusOutlined />,
								type: 'primary'
							}}
...
export default StockingBomItemProfile;
