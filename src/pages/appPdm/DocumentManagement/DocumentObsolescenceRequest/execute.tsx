...
import { DocumentObsolescenceRequestGetAsync, DocumentObsolescenceRequestExecuteWorkflowAsync } from '@/services/pdm/DocumentObsolescenceRequest';
...
const ExecuteFormPage = (props: any) => {
  const { params } = useKeepAliveParams('/appPdm/DocumentManagement/DocumentObsolescenceRequest/execute', ['definitionId', 'correlationId', 'activityId', 'workflowInstanceId', 'workItemId']);
        ...
        effects: () => {
          onFormInit(async (form) => {
            if (correlationId) {
              setLoading(true);
              try {
                let values: any = await DocumentObsolescenceRequestGetAsync({ id: correlationId });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await DocumentObsolescenceRequestExecuteWorkflowAsync({ id: correlationId }, submitData);
  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', padding: '16px' }}>
      ...
      {/* 基本信息卡片 */}
      <Card title="文档作废申请信息" style={{ marginBottom: 16 }}>
