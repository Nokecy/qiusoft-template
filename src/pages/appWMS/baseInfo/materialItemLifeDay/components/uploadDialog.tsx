...
const UploadDialog = (props: any) => {
    ...
    return <>
        ...
        <Modal title={"批量导入物料"} open={visible} onCancel={setFalse} destroyOnClose>
            <Alert message={<span>点击下载 <a onClick={() => { downloadBlob("/api/wms/materialitem-lifeday/import-template", "批量上传库房存储期模版.xlsx") }}>模版</a></span>} type={"info"} />

            <Upload.Dragger style={{ marginTop: 10 }} multiple={false}
                action={`${serverUrl()}/api/wms/materialitem-lifeday/import`}
                onChange={(info) => {
                    if (info.file.status === "done") {
                        if(info.file.response.length){
                            message.error(info.file.response[0]);
                        }else{
