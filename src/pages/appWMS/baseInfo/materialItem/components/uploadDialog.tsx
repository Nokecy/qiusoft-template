...
const UploadDialog = (props: any) => {
    ...
    return <>
        ...
        <Modal title={"批量导入物料"} open={visible} onCancel={setFalse} destroyOnClose>
            <Alert message={<span>点击下载 <a onClick={() => { downloadBlob("/api/wms/materialItem/import-template", "批量上传物料模版.xlsx") }}>模版</a></span>} type={"info"} />

            <Upload.Dragger style={{ marginTop: 10 }} multiple={false}
                action={`${serverUrl()}/api/wms/materialItem/import`}
                onChange={(info) => {
