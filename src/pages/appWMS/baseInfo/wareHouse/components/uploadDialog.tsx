...
const UploadDialog = (props: any) => {
	...
	return (
		<>
			...
			<Modal title={'批量导入库房'} open={visible} onCancel={setFalse} destroyOnClose>
				<Alert
					message={
						<span>
							点击下载{' '}
							<a
								onClick={() => {
									downloadBlob(`/api/wms/warehouse/import-template`, '批量上传库房模版.xlsx');
								}}
							>
								模版
							</a>
				...
				<Upload.Dragger
					style={{ marginTop: 10 }}
					multiple={false}
					action={`${serverUrl()}/api/wms/warehouse/import`}
					onChange={info => {
