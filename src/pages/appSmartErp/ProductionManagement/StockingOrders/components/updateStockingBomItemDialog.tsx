import { StockingOrderGetBomItemAsync, StockingOrderUpdateBomItemAsync } from '@/services/smarterp/StockingOrder';
...
const UpdateFormDialog: React.FC<UpdateFormDialogProps> = (props) => {
	...
	const portalId = `UpdateStockingBomItemDialog${entityId}`;
	...
	const handleClick = async () => {
		...
		if (!rowData || Object.keys(rowData).length === 0) {
			if (entityId) {
				try {
					formData = await StockingOrderGetBomItemAsync({ bomItemId: entityId });
				} catch (error) {
		...
		formDialog
			.forConfirm((payload, next) => {
				...
				// 只传递 UpdateStockingBomItemDto 需要的字段
				const updateData = {
					id: entityId,
					qty: values.qty,
					memo: values.memo
				...
				return StockingOrderUpdateBomItemAsync(updateData).then(() => {
