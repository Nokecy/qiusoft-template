...
import { StockingOrderCreateBomItemAsync } from '@/services/smarterp/StockingOrder';
...
const CreateFormDialog: React.FC<CreateFormDialogProps> = (props) => {
	const { entityId, title, type, buttonProps, selectRow, stockingOrderId, stockingOrderItemId, onAfterSubmit, children } = props;
	...
	const portalId = `CreateStockingBomItemDialog${type}${entityId || ''}`;
				...
				onClick={() => {
					...
					formDialog
						.forConfirm((payload, next) => {
							...
							// 确保数据结构符合 CreateStockingBomItemDto
							const createData = {
								stockingOrderId: values.stockingOrderId,
								stockingOrderItemId: values.stockingOrderItemId,
							...
							return StockingOrderCreateBomItemAsync(createData).then(() => {
						...
						.open({
							effects: () => {
								onFormInit(async (form) => {
									if (selectRow) {
										// 使用正确的字段名，与 StockingOrderItemWorkBomItemDto 一致
