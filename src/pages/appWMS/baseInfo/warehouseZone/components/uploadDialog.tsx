...
const UploadDialog = (props: any) => {
	...
	return (
		<>
			...
			<Modal title={'批量导入库区'} open={visible} onCancel={setFalse} destroyOnClose>
				<Alert
					message={
						<span>
							点击下载{' '}
							<a
								onClick={() => {
									downloadBlob(`/api/wms/warehouse-zone/import-template`, '批量上传库区模版.xlsx');
								}}
							>
								模版
							</a>
				...
				<Upload.Dragger
					style={{ marginTop: 10 }}
					multiple={false}
					action={`${serverUrl()}/api/wms/warehouse-zone/import`}
					onChange={info => {
