...
import { OutInspectionScheme } from '@/services/wmsPermission';
...
import { OutInspectionSchemeDeleteAsync, OutInspectionSchemeGetListAsync } from '@/services/wms/OutInspectionScheme';
import OutInspectionSchemeFormDialog from './components/formDialog';
const Options = (props: ICellRendererParams & { onRefresh }) => {
	...
	const handleDelete = (id: any) => {
		...
		return OutInspectionSchemeDeleteAsync({ id })
			.then(() => {
				refresh();
			})
			.finally(() => {
	...
	return (
		<Space>
			<Access accessible={access[OutInspectionScheme.Update]}>
				<OutInspectionSchemeFormDialog
					title={'编辑物料'}
					entityId={data.id}
					onAfterSubmit={() => {
						refresh();
			...
			<Access accessible={access[OutInspectionScheme.Delete]}>
...
const OutInspectionSchemePage = (props: any) => {
	...
	return (
		<>
			<AgGridPlus
				gridRef={gridRef}
				headerTitle={'出库检验方案列表'}
				gridKey="appWMS.baseInfo.outInspectionScheme"
				request={async (params?: { pageSize: number; current: number; [key: string]: any }) => {
					let data = await OutInspectionSchemeGetListAsync({ Filter: params?.filter, Sorting: params!.sorter, SkipCount: params!.skipCount, MaxResultCount: params!.maxResultCount });
					let requestData: any = { success: true, data: data.items!, total: data.totalCount };
					return requestData;
				}}
				toolBarRender={gridApi => {
					return [
						<Access accessible={access[OutInspectionScheme.Create]}>
							<OutInspectionSchemeFormDialog title={'新增方案'} onAfterSubmit={onRefresh}>
								新增方案
							</OutInspectionSchemeFormDialog>
						</Access>,
					];
...
export default OutInspectionSchemePage;
