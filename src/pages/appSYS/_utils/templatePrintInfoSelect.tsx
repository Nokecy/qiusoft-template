...
import { LabelPrintTemplateGetListAsync } from '@/services/openApi/LabelPrintTemplate';
...
const getData = ({ Filter, Sorting, SkipCount, MaxResultCount }: any) => {
    return LabelPrintTemplateGetListAsync({
...
const TemplatePrintInfoSelect = (props: SelectProps<any> & { disabledKeys?: any[]; showId?: boolean; code: boolean }, ref) => {
    ...
    return (
        <span ref={ref}>
            ...
            >
                {data?.items?.map(i => (
                    <Option value={code ? i.code! : i.id!}>{i.name}</Option>
                ))}
            </DialogSelect>
        </span>
    );
...
export default React.forwardRef(TemplatePrintInfoSelect);
