...
const MaterailItemSelect = (props: any & { disabledKeys?: any[]; showId?: boolean; filter?: any }, ref) => {
	const { showId = true, outCode } = props;
		...
		<span ref={ref}>
			<DialogSelect
				...
				onChange={(selectedValue, option) => {
					// 如果是labelInValue模式，需要附加完整的数据对象
					if (selectedValue && typeof selectedValue === 'object') {
						// 从数据源中找到对应的完整数据
						const selectedData = data?.items?.find(item => item.id === selectedValue.value);
						const enrichedValue = {
							...selectedValue,
							data: selectedData // 添加完整的数据对象
						};
						setState(enrichedValue);
					} else {
				...
				labelInValue
				valueField={'id'}
				labelField={outCode ? 'outCode' : 'code'}
				{...props}
			...
			>
				{
					data && outCode ? data.items!.map(materialItem => (
						<Option key={materialItem.code + '&' + materialItem.outCode} value={materialItem.id!}>{materialItem.outCode}</Option>
					)) :
						data && showId ? data.items!.map(materialItem => (
